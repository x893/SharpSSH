 ...
 */
	public class PipedInputStream : SharpSsh.java.io.InputStream
	{
		 ...
		 * @since   JDK1.1
		 */
		internal byte[] m_buffer = new byte[PIPE_SIZE];
		...
		internal void receive(int b) 
		{
			...
			m_buffer[m_in++] = (byte)(b & 0xFF);
			if (m_in >= m_buffer.Length) 
				m_in = 0;
		}
		...
		internal void receive(byte[] b, int off, int len) 
		{
			...
			while (bytesToTransfer > 0) 
			{
				...
				if (m_out < m_in) 
					nextTransferAmount = m_buffer.Length - m_in;
				else if (m_in < m_out) 
				{
					if (m_in == -1) 
					{
				...
				Array.Copy(b, off, m_buffer, m_in, nextTransferAmount);
		...
		public override int Read()
		{
			if (!connected) 
				...
				throw new IOException("Pipe closed");
			else if (writeSide != null && !writeSide.IsAlive && !closedByWriter && (m_in < 0))
			...
			while (m_in < 0) 
			{
				if (closedByWriter)	/* closed by writer, return EOF */
					return -1;
			...
			int ret = m_buffer[m_out++] & 0xFF;
			if (m_out >= m_buffer.Length) 
				m_out = 0;

			if (m_in == m_out)	/* now empty */
				m_in = -1;
		...
		public virtual int available() 
		{
			if(m_in < 0)
				...
				return m_in + m_buffer.Length - m_out;
		}
		...
		public override long Length
		{
			get
			{
				...
				{
					return (m_buffer.Length -m_out+m_in);
				}
			}
		}
