...
namespace SharpSsh
{
	...
	public class SshShell : SshBase
	{
		...
		private static string m_escapeCharsPattern = "\\[[0-9;?]*[^0-9;]";
		...
		public string Expect(Regex pattern)
		{
			...
			do
			{
				...
				sb_response.Append(tmp, 0, readCount);
				string response = sb_response.ToString();
				match = pattern.Match(response);
			} while (!match.Success);

			return (RemoveTerminalEmulationCharacters
					? HandleTerminalChars(sb_response.ToString())
		...
		/// </summary>
		public static string HandleTerminalChars(string response)
		{
			response = response.Replace("(B)0", "");
			response = Regex.Replace(response, m_escapeCharsPattern, "");
			response = response.Replace(((char)15).ToString(), "");
			response = Regex.Replace(response, ((char)27) + "=*", "");
