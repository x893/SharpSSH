...
namespace SharpSsh
{
	...
	public class SshStream : Stream
	{
		...
		public SshStream(string host, string username, string password)
		{
			this.m_host = host;
			JSch jsch = new JSch();
			m_session = jsch.getSession(username, host, SharpSsh.SshBase.SSH_TCP_PORT);
		...
		public string ReadResponse()
		{
			...
			{
				readCount = Read(buff);
				sb_response.Append(Encoding.Default.GetString(buff), 0, readCount);
				string response = sb_response.ToString();
				match = m_prompt.Match(response);
			} while (!match.Success);

			return HandleTerminalChars(sb_response.ToString());
		}
		...
		/// <returns></returns>
		private string HandleTerminalChars(string response)
		{
			if (RemoveTerminalEmulationCharacters)
			{
				response = response.Replace("(B)0", "");
				response = Regex.Replace(response, m_escapeCharPattern, "");
				response = response.Replace(((char)15).ToString(), "");
				response = Regex.Replace(response, ((char)27).ToString() + "=*", "");
